...
maryland_ppp %>%
  ...
  # select basically tells you what values it wants to see, in this case loan number, amount, payroll proceed
  # mutate(name_of_column = value)
  # PALM BEACH != Palm Beach so we do
...
maryland_ppp %>%
  ...
  # finds payroll percentage then puts it in order from smallest to largest
```
What is the payroll proceeds percentage for the first result? Put your answer in the space below.
**Answer** 0%
...
maryland_ppp %>%
  ...
  # syntax for filter --> filter(name of what your looking for == value of what your looking for)
...
maryland_with_in_out <- maryland_ppp %>%
  # maryland_with_in_out serves as new data frame
  mutate(
    in_out = if_else(
        servicing_lender_state == 'MD', "IN", "OUT"
      )
  )
  # in_out serves as new column, if servicing lender state is MD then IN, if not MD then OUT
```
