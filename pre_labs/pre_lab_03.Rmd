...
# changes the id from numeric to character using above command
glimpse(ppp_maryland_loans)
```
...
ppp_maryland_loans %>%
  ...
  # you cant do filter(forgiveness_date == 'NA')
  # you can filter to find where things are NOT NA by adding a ! at the beginning
  arrange(desc(count))
```
...
**Answer**: 135073. They are the same number. # what does this mean
...
**Answer** 10000000 and 6. 10000000 isn't that suspicious because it is the max loan amount possible but 6 seems strange because it seems like such an inconsequential loan amount.
```{r}
